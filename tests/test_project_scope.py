...
def test_pom_xml_and_evidence_anchor_paths_do_not_get_mismatch_penalty() -> None:
    ...
    assert project_scope_score_parts(
        _chunk("backend/pom.xml", project_root="backend", project_name="backend", project_kind="java"),
        scope,
        project_unit_count=2,
    ) == {}
    assert project_scope_score_parts(
        _chunk("docs/risks.md", project_root="", project_name="repo", project_kind="unknown"),
        scope,
        project_unit_count=2,
    ) == {}
