...
def build_parser() -> argparse.ArgumentParser:
    ...
    merge_checkpoints = subparsers.add_parser(
        "merge-checkpoints",
        help="merge one-sample benchmark checkpoint shards",
    )
    merge_checkpoints.add_argument(
        "--input", type=Path, action="append", required=True
    )
    merge_checkpoints.add_argument("--output", type=Path, required=True)
...
def merge_benchmark_checkpoints(
    inputs: Iterable[str | Path],
    output: str | Path,
) -> Path:
    directories = [Path(value).resolve() for value in inputs]
    if not directories:
        raise ValueError("benchmark checkpoint merge requires inputs")
    output_path = Path(output).resolve()
    if os.path.lexists(output_path):
        raise FileExistsError(output_path)

    sample_schema = _load_json(
        SCHEMA_ROOT / "benchmark-report-v1.json"
    )["properties"]["samples"]["items"]
    metadata_values: list[dict[str, Any]] = []
    samples: dict[int, dict[str, Any]] = {}
    expected_identity: Mapping[str, Any] | None = None
    expected_request: Mapping[str, Any] | None = None
    expected_stable_environment: Mapping[str, Any] | None = None

    for directory in directories:
        if not directory.is_dir():
            raise ValueError("benchmark checkpoint shard is not a directory")
        visible = sorted(
        ...
        if {path.name for path in visible} != {
            "run.json",
            *(path.name for path in sample_paths),
        } or len(sample_paths) != 1:
            raise ValueError("benchmark checkpoint shard must contain one sample")

        metadata = _load_json(directory / "run.json")
        ...
        if (
            ...
            or metadata["checkpoint_kind"] != "p6-benchmark-run-v1"
            or not isinstance(metadata["identity"], dict)
            or not isinstance(metadata["request"], dict)
            or not isinstance(metadata["environment"], dict)
        ):
            raise ValueError("benchmark checkpoint metadata is invalid")
        try:
            stable_environment = _checkpoint_stable_environment(
                metadata["environment"]
            )
        except KeyError as exc:
            raise ValueError("benchmark checkpoint environment is incomplete") from exc
        stable_environment_sha256 = hashlib.sha256(
            canonical_json(stable_environment).encode("utf-8")
        ).hexdigest()
        if (
            metadata["identity"].get("stable_environment_sha256")
            != stable_environment_sha256
        ):
            raise ValueError("benchmark checkpoint environment identity differs")

        if expected_identity is None:
            expected_identity = metadata["identity"]
            expected_request = metadata["request"]
            expected_stable_environment = stable_environment
        elif (
            metadata["identity"] != expected_identity
            or metadata["request"] != expected_request
            or stable_environment != expected_stable_environment
        ):
            raise ValueError("benchmark checkpoint shard identity differs")

        match = re.fullmatch(r"sample-(\d{3})\.json", sample_paths[0].name)
        if match is None:
            raise AssertionError("validated checkpoint sample name did not match")
        index = int(match.group(1))
        if index in samples:
            raise ValueError("benchmark checkpoint shard sample is duplicated")
        samples[index] = _validate_checkpoint_sample(
            _load_json(sample_paths[0]),
            index=index,
            identity_sha256=_checkpoint_identity_sha256(metadata),
            sample_schema=sample_schema,
        )
    ...
    if expected_request is None:
        raise AssertionError("checkpoint merge lost its request")
    sample_count = expected_request.get("sample_count")
    if type(sample_count) is not int or sample_count < 1:
        raise ValueError("benchmark checkpoint sample count is invalid")
    if set(samples) != set(range(1, sample_count + 1)):
        raise ValueError("benchmark checkpoint shards are incomplete")

    merged_metadata = dict(metadata_values[0])
    merged_environment = dict(metadata_values[0]["environment"])
    merged_environment["background_cpu_percent"] = max(
        float(value["environment"]["background_cpu_percent"])
        for value in metadata_values
    )
    merged_environment["swap_before_bytes"] = min(
        int(value["environment"]["swap_before_bytes"])
        for value in metadata_values
    )
    merged_environment["swap_after_bytes"] = max(
        int(value["environment"]["swap_after_bytes"])
        for value in metadata_values
    )
    merged_metadata["environment"] = merged_environment

    output_path.mkdir(parents=True)
    _write_new_json_atomic(output_path / "run.json", merged_metadata)
...
def run_benchmark(
    ...
    *,
    ...
    if shard_index is not None and (
        ...
        or resume
        or shard_index < 1
        or shard_index > sample_count
    ):
        raise ValueError(
            "sharded measurement requires a fresh checkpoint and valid sample index"
        )
    ...
    if shard_index is not None and unsupported:
        raise ValueError("unsupported benchmark measurements cannot be sharded")
    ...
    with tempfile.TemporaryDirectory(
        ...
        sample_indices: Iterable[int] = (
            ...
            else range(len(samples), sample_count if supported else 0)
        )
    ...
    if shard_index is not None:
        _benchmark_progress(
            f"shard sample {shard_index}/{sample_count} complete"
        )
        return None
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        if args.command == "generate":
            ...
            merge_benchmark_checkpoints(args.input, args.output)
