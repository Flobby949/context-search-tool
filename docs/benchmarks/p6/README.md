...
For cloud runners with a per-job time limit, `run --shard-index N` executes
exactly one registered sample while retaining the frozen total `--samples`
identity. Each shard still performs its own identical warmup and measured
operation and writes no partial benchmark report. `merge-checkpoints` accepts
only one complete, non-duplicated set of sample shards whose implementation,
request, workload, repository, and stable runner environment identities match.
The merged checkpoint is finalized with the ordinary `run --resume` path.
The five-way smoke workflow is diagnostic when the available cloud runner does
not satisfy the report's minimum host-validity requirements; the report keeps
those validity reasons rather than treating parallel completion as formal
